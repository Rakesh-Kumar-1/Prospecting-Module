...
const logDelivery = async (
  queueRow, result, status, attemptNumber
) => {
  try {
    await db.query(
    ...
    logger.error(
      `[WORKER] Failed to write td_message_logs: ${logErr.message}`
    );
  }
};
  ...
  try {
    ...
    for (const msg of messages) {
      ...
      try {
        ...
        logger.info(
          `[WORKER] ✅ SENT | ID: ${msg.id} | ${msg.channel} → ${msg.to_address}`
        );

      } catch (sendErr) {
        const newRetryCount = msg.retry_count + 1;
        const finalStatus = newRetryCount >= msg.max_retries
          ? 'FAILED'
          : 'PENDING';
        ...
        logger.error(
          `[WORKER] ❌ FAILED | ID: ${msg.id} | Attempt ${newRetryCount}/${msg.max_retries} | ${sendErr.message}`
        );
      }
    }
  } catch (err) {
...
logger.info(
  `[WORKER] 🚀 Started — polling every ${config.worker.pollMs}ms`
);
