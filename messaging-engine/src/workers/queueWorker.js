  ...
  try {
    await db.query(
      `INSERT INTO message_logs
       (queue_id, channel, to_address, provider, provider_msg_id, status_code, response_body, delivered_at, attempt_number)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        queueRow.id,
        queueRow.channel,
    ...
    logger.error(`[WORKER] Failed to write message_logs: ${logErr.message}`);
  }
};

const routeToProvider = async (msg) => {
  // Safely parse payload — handle both string and object
  let payload;
  ...
  try {
    ...
    for (const msg of messages) {
      const [locked] = await db.query(
        `UPDATE message_queue SET status = 'PROCESSING', last_attempt_at = NOW()
      ...
      try {
        ...
        logger.info(`[WORKER] ✅ SENT | ID: ${msg.id} | ${msg.channel} → ${msg.to_address}`);

      } catch (sendErr) {
        const newRetryCount = msg.retry_count + 1;
        const finalStatus = newRetryCount >= msg.max_retries ? 'FAILED' : 'PENDING';
        await db.query(
          `UPDATE message_queue 
           SET status = ?, retry_count = ?, error_message = ? 
           WHERE id = ?`,
          [finalStatus, newRetryCount, sendErr.message, msg.id]
        );
